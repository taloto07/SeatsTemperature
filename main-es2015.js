...
const credentialsIncorrectErrorMessage = "Credentials are incorrect!";
const crossOriginErrorMessage = "Cross origin restriction!";
...
let SeatItemComponent = class SeatItemComponent {
    constructor(command) {
        this.command = command;
        this.seatPropertyNames = ["seat_heater_left", "seat_heater_right"];
    }
    set disableSeat(disableSeats) {
        ...
        if (this.isFrontSeat(this.seat.number)) {
            this.command.getVehicleState(this.vehicleId).subscribe(res => {
            ...
            this.command.setSeatHeater(this.seat.number, 0, this.vehicleId).subscribe(data => {
                this.seat.level = 0;
            }, err => {
                console.log(this.seat.number, err);
            });
    ...
    increaseLevel() {
        ...
        this.command.setSeatHeater(this.seat.number, seatLevel, this.vehicleId).subscribe(data => {
            console.log(seatLevel);
    ...
    isFrontSeat(seatNumber) {
        return seatNumber === 0 || seatNumber === 1;
    }
};
SeatItemComponent.ctorParameters = () => [
...
let SeatsComponent = class SeatsComponent {
    constructor(command) {
        this.command = command;
    ...
    ngOnInit() {
        ...
        this.command.getVehicles().subscribe(res => {
    ...
    startClimateControl() {
        this.command.startClimateControlSystem(this.vehicleId).subscribe(res => {
            console.log(res);
            this.climate.disabled = false;
    ...
    stopClimateControl() {
        this.command.stopClimateControlSystem(this.vehicleId).subscribe(res => {
            console.log(res);
            this.climate.disabled = false;
...
let CommandService = class CommandService {
    ...
    getVehicles() {
        let url = `${this.baseUrl}/${this.vehiclesUrl}`;
        return this.http.get(url);
    }
    wakeUpVehicle(id) {
        let wakeUrl = `${this.baseUrl}/${this.vehiclesUrl}/${id}/wake_up`;
        return this.http.post(wakeUrl, {});
    }
    getVehicleState(id) {
        let stateUrl = `${this.baseUrl}/${this.vehiclesUrl}/${id}/vehicle_data`;
        return this.http.get(stateUrl);
    }
    setSeatHeater(seatNumber, level, id) {
